...
myWords <- data.frame(word=c("hodge","reshi","warne","tpadz","vicky","chris","lukey","emilo","clare","ellie","place"),key=rep(17:27))
myWords <- subset(myWords,myWords$key==as.numeric(format(Sys.Date(),"%d")))$word


server <- function(input, output) {
  target_word <- reactiveVal(sample(myWords, 1))
  ...
  observeEvent(input$Enter, {
    ...
    if (! guess %in% c(words_all,myWords))
      return()
